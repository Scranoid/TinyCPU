                    ...
                    Text(instr.name)
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: 14)
                                .stroke(isActive ? Color.accentBlue.opacity(0.8) : (isPast ? Color.green.opacity(0.28) : Color.clear), lineWidth: isActive ? 2 : 1)
                        )
                        .foregroundColor(theme.isDarkMode ? .white : .black)
                        .opacity(isPast ? 0.55 : 1.0)
                        .scaleEffect(isActive ? 1.08 : 1.0)
                        .dynamicShadow(tiltX: tilt.tiltX, tiltY: tilt.tiltY)
                        .animation(.easeInOut(duration: 0.22), value: currentPC)
                }
            }
            .padding(.vertical, 8)
        }
